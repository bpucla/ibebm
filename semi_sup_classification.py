...
def train(args_job, output_dir_job, output_dir, return_dict):
    ...
    def sample_langevin_prior_z(z, netE, verbose=False, y=None):
        z = z.clone().detach()
        ...
        for i in range(args.e_l_steps):
            if y is None:
                ...
                en = energy(netE(z, cls_output=True))[range(z.size(0)), y]
            ...
            if (i % 5 == 0 or i == args.e_l_steps - 1) and verbose:
                if y is None:
                    ...
                    logger.info('Conditional Langevin prior {:3d}/{:3d}: energy={:8.3f}'.format(i + 1, args.e_l_steps, en.sum().item()))

            z_grad_norm = z_grad.view(z_grad.size(0), -1).norm(dim=1).mean()

        return z.detach(), z_grad_norm

    def sample_langevin_post_z(z, x, netG, netE, verbose=False, y=None):
        z = z.clone().detach()
        z.requires_grad = True
        for i in range(args.g_l_steps):
            ...
            if args.g_l_with_noise:
                z.data += args.noise_factor * args.g_l_step_size * torch.randn_like(z).data
            ...
            if (i % 5 == 0 or i == args.g_l_steps - 1) and verbose:
                if y is None:
                    ...
                    logger.info('Conditional Langevin posterior {:3d}/{:3d}: MSE={:8.3f}'.format(i+1, args.g_l_steps, g_log_lkhd.item()))
    ...
    def classification(dataloader, netI, netE):
        ...
        for i, (x, y) in enumerate(dataloader):
            ...
            mu, _ = netI(x)
            scores = netE(mu, cls_output=True)
    ...
    bg_freq = torch.tensor(bg_freq).to(device)
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (x, _) in enumerate(dataloader_train, 0):
            ...
            loss_g_kl = torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim=1), dim=0).squeeze()
            loss_g_ebm_cross_ent = - netE(z_g_k).squeeze().mean()
            mi = netE.compute_mi(z_g_k)
            loss_g = loss_g_recon + loss_g_kl + loss_g_ebm_cross_ent - 100 * mi
            loss_g.backward()
            grad_norm_g = get_grad_norm(netG.parameters())
            grad_norm_i_unlabeled = get_grad_norm(netI.parameters())
            if args.g_is_grad_clamp:
                torch.nn.utils.clip_grad_norm(netG.parameters(), args.g_max_norm)
            if args.i_is_grad_clamp:
                torch.nn.utils.clip_grad_norm(netI.parameters(), args.i_max_norm)
            optG.step()
            optI.step()

            if args.ebm_update_unlabeled:
                # Learn prior EBM
                optE.zero_grad()
                en_neg = energy(netE(z_e_k.detach())).mean()
                en_pos = energy(netE(z_g_k.detach())).mean()
                mi = netE.compute_mi(z_g_k.detach())
                loss_e = en_pos - en_neg - 100 * mi
                loss_e.backward()
            ...
            if i % args.n_printout == 0:
                with torch.no_grad():
                    ...
                    logger.info('{} {:5d}/{:5d} {:5d}/{:5d} '.format(job_id, epoch, args.n_epochs, i, len(dataloader_train)) +
                        ...
                        'loss_g_recon={:8.3f}, '.format(loss_g_recon) +
                        'loss_g_kl={:8.3f}, '.format(loss_g_kl) +
                        'loss_g_ebm_cross_ent={:8.3f}, '.format(loss_g_ebm_cross_ent) +
                        'cls_loss_e={:8.3f}, '.format(cls_loss / args.labeled_weight) +
                        'train_cls_acc={:8.3f}, '.format(torch.stack(train_acces).mean().item()) +
                        ...
                        '|grad_e_labeled|={:8.2f}, '.format(grad_norm_e_labeled) +
                        '|grad_i_unlabeled|={:7.3f}, '.format(grad_norm_i_unlabeled) +
                        '|grad_i_labeled|={:7.3f}, '.format(grad_norm_i_labeled) +
        ...
        if epoch % args.n_cls_validataion == 0:
            ...
            logger.info('validation classification accuracy={:8.6f}'.format(val_acc))


        # Schedule
        lr_scheduleE.step(epoch=epoch)
        lr_scheduleG.step(epoch=epoch)
        lr_scheduleI.step(epoch=epoch)
        ...
        if epoch % args.n_stats == 0:
            ...
            stats['cls_loss'].append(torch.stack(cls_losses).mean().item())
            stats['cls_acc'].append(val_acc.item())
